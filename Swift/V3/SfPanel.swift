...
class SfPanel: NSObject {
    ...
    func setSize(_ width:Float!, height:Float!) -> SfPanel {
        
        self.size.setSize(width, height: height);
        return self;
    }
    
    func setPosition(_ position:Int!) -> SfPanel {
        self.position = position;
        return self;
    }
    
    func setAlignment(_ alignment:Int!) -> SfPanel {
        self.alignment = alignment;
        return self;
    }
    
    func setOrigin(_ top:Float!, right:Float!, bottom:Float!, left:Float!) -> SfPanel {
        self.origin.setBox(top, right: right, bottom: bottom, left: left);
        return self;
    }
    
    func setMargin(_ top:Float!, right:Float!, bottom:Float!, left:Float!) -> SfPanel {
        self.margin.setBox(top, right: right, bottom: bottom, left: left);
        return self;
    }
    
    func setPadding(_ top:Float!, right:Float!, bottom:Float!, left:Float!) -> SfPanel {
        self.padding.setBox(top, right: right, bottom: bottom, left: left);
        return self;
    }
    
    @objc(methodNewSetterParent:)
    func setParent(_ parent:SfPanel!) -> SfPanel {
    ...
    func setKey(_ key:NSString!) -> SfPanel {
        self.key = key;
        return self;
    }
    ...
    func append(_ object:SfPanel!) -> SfPanel {
    ...
    func prepend(_ object:SfPanel!) -> SfPanel {
    ...
    func setZIndex(_ z:Int!) -> SfPanel {
        self.zIndez = z;
        return self;
    }
    
    func setVisible(_ vis:Bool!) -> SfPanel {
        self.visible = vis;
        return self;
    }
    ...
    func find(_ key:NSString!) -> SfPanel {
        ...
        while ( child != nil ) {
            ...
            if ( child.getKey().isEqual(to: key as String) ) {
                ret = child;
                break;
            } else {
                ret = child.find(key);
    ...
    func closest(_ key:NSString) -> SfPanel {
        
        var ret:SfPanel! = SfPanel();
        
        if ( self.parent != nil ) {
            
            if ( self.parent.getKey().isEqual(to: key as String) ) {
                ret = self.parent;
            } else {
    ...
    func calcPos() {
        ...
        while ( panel != nil ) {
            ...
            case SF_POSITION_ABSOLUTE:
                ...
                panel.frame.x = (panel.origin.left != Float(SF_UNSET)) ? (Float(self.frame.x) + Float(panel.origin.left) + Float(panel.margin.left) ) : (parentRight - (panelW + panel.origin.right));
                //}
                ...
                panel.frame.y = (panel.origin.top != Float(SF_UNSET)) ? ( Float(self.frame.y) + Float(panel.origin.top) + Float(panel.margin.top)) : (parentBotton - (panelH + panel.origin.bottom));
                //}
        ...
        if ( self.alignment == SF_ALIGNMENT_CENTER ) {
            ...
            var lineWidths = [NSInteger!](repeating: nil, count: line+1);
            for i in 0 ..< (line+1) {
                lineWidths[i] = 0;
            }
            
            var lineHeights = [NSInteger!](repeating: nil, count: line+1);
            for i in 0 ..< (line + 1) {
                lineHeights[i] = 0;
            }
    ...
    func update() {
        ...
        let rect:CGRect! = CGRect(x: CGFloat(self.frame.x), y: CGFloat(self.frame.y), width: CGFloat(self.frame.width), height: CGFloat(self.frame.height));
